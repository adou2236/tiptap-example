    ...
    <el-collapse-item title="数据" name="data">
      <el-form size="mini" label-position="left">
        ...
        >
          ...
          <el-form-item label="选择地域：" required>
            <el-radio-group v-model="selectedVar">
              <el-radio label="currentArea">当前地区</el-radio>
              <el-radio v-for="(item,index) in radios" :key="index"  :disabled="count>1" :label="item.id">{{item.label}}</el-radio>
            </el-radio-group>
          </el-form-item>
...
import SeriesTimeOption from "./series-time-option.vue";
